  ...
  return (
    <section id="inicio" ref={ref} className="relative w-full h-screen flex flex-col justify-center">
      ...
      <div className="relative z-10 px-14 sm:px-20 2xl:px-96 text-black">
        <motion.div
          className="sm:w-3/4 2xl:w-1/2  p-8 rounded-lg "
          initial={{ opacity: 0, x: -150 }}
          animate={isInView ? { opacity: 1, x: 0 } : { opacity: 0, x: -150 }}
          transition={{ duration: 1.5, ease: "easeOut" }}
        >
          <h1 className="text-4xl sm:text-5xl 2xl:text-5xl font-bold">
            Desarrollador Frontend & Diseñador Web
          </h1>
          <p className="text-lg sm:text-lg 2xl:text-xl mt-4 pb-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between sm:w-[270px] xl:w-[270px]">
            <StyledButton 
              ...
              className="mt-2 bg-white text-black px-4 py-3 w-32 sm:w-[130px] text-center rounded-md text-sm font-medium transition   hover:text-white "
            >
              Ver proyectos
            </StyledButton>
            <StyledButton
